...
export function valueFromOption<T>(
  value: T | PropOptionValue<T>,
): T | undefined | null {
...
export function valuesFromOptions<T>(
  value: unknown | T[] | PropOptions<T>,
): T[] {
  if (
    typeof value === "object" &&
    value &&
...
export function arrayPropErrors(
  opts: ValidationOpts<ConfigurablePropStringArray>,
): string[] | undefined {
  const _values = valuesFromOptions(opts.value);
  if (!opts.prop.default && typeof _values === "undefined") {
    return ["required"];
  }
  if (!opts.prop.default && Array.isArray(_values) && !_values.length)
    return ["empty array"];
}

export function booleanPropErrors(
  opts: ValidationOpts<ConfigurablePropBoolean>,
): string[] | undefined {
  const _value = valueFromOption(opts.value);
  if (_value == null || typeof _value === "undefined") return ["required"];
}

export function integerPropErrors(
  opts: ValidationOpts<ConfigurablePropInteger>,
): string[] | undefined {
  const { prop, value: valueOpt } = opts;
  const value = valueFromOption(valueOpt);

  if (!prop.default && (value == null || typeof value === "undefined"))
    return ["required"];

  const _value: number =
    typeof value === "number" ? value : parseInt(String(value));

  if (Number.isNaN(_value)) return ["not a number"];
  const errs = [];
  if (typeof prop.min === "number" && _value < prop.min)
    errs.push("number too small");
  if (typeof prop.max === "number" && _value > prop.max)
    errs.push("number too big");
  return errs;
}

export function stringPropErrors(
  opts: ValidationOpts<ConfigurablePropString>,
): string[] | undefined {
  const { prop, value: valueOpt } = opts;

  if (!prop.default && (valueOpt == null || typeof valueOpt === "undefined"))
    return ["required"];
}
...
type AppPropValueWithCustomFields<T extends AppCustomField[]> =
  PropValue<"app"> & {
...
function getCustomFields(app: App): AppCustomField[] {
  ...
  if (
    "extracted_custom_fields_names" in app &&
    app.extracted_custom_fields_names
  ) {
    const extractedCustomFields = (
      (app as AppWithExtractedCustomFields).extracted_custom_fields_names || []
    ).map((name) => ({
      name,
    }));
...
export function appPropErrors(
  opts: ValidationOpts<ConfigurablePropApp>,
): string[] | undefined {
  ...
  if ("authProvisionId" in _value && !_value.authProvisionId) {
    if (app.auth_type) {
      ...
      if (
        app.auth_type === "oauth" &&
        !(_value as OauthAppPropValue).oauth_access_token
      ) {
      ...
      if (app.auth_type !== "none") errs.push("no auth provision configured");
      return errs;
    }
  }
}
