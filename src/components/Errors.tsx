...
export function Errors<T extends ConfigurableProps, U extends ConfigurableProp>(
  props: ErrorsProps<T, U>,
) {
  const { field } = props;
  const { errors = {}, prop = {}, enableDebugging } = field;
  ...
  const formattedErrors: ConfigurablePropAlert[] = errors[prop.name].map(
    (e) => {
      return {
        type: "alert",
        alertType: "error",
  ...
  return (
    <div className="pd-errors" style={baseStyles}>
      <FormattedErrors />
    </div>
  );
}
